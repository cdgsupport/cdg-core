...
class CDG_Core_Admin
{
    ...
    public function render_settings_page(): void
    {
        ...
        <div class="wrap cdg-core-settings">
            <h1><?php echo esc_html(get_admin_page_title()); ?> <small>v<?php echo esc_html(CDG_CORE_VERSION); ?></small></h1>

            <nav class="nav-tab-wrapper">
                <?php foreach ($tabs as $id => $name): ?>
                    <a href="<?php echo esc_url(
                        admin_url('options-general.php?page=cdg-core-settings&tab=' . $id),
                    ); ?>"
            ...
            <form method="post" action="<?php echo esc_url(
                ...
                <p class="submit">
                    <input type="submit" name="cdg_core_save_settings" class="button button-primary" value="<?php esc_attr_e('Save Changes', 'cdg-core'); ?>">
                </p>
            </form>
        </div>
        <?php
    ...
    private function tab_features(array $s): void
    {
        ...
        <table class="form-table">
            <tr>
                <th><?php esc_html_e('Documentation System', 'cdg-core'); ?></th>
                <td>
                    <label><input type="checkbox" name="enable_documentation" value="1" <?php checked($s['enable_documentation']); ?>> <?php esc_html_e('Enable internal documentation', 'cdg-core'); ?></label>
                    <div style="margin: 10px 0 0 24px;">
                        <label><input type="checkbox" name="show_documentation_widgets" value="1" <?php checked($s['show_documentation_widgets']); ?>> <?php esc_html_e('Show dashboard widgets', 'cdg-core'); ?></label><br><br>
                        <strong><?php esc_html_e('Widget Style:', 'cdg-core'); ?></strong><br>
                        <label><input type="radio" name="documentation_module_style" value="informative" <?php checked($s['documentation_module_style'], 'informative'); ?>> <?php esc_html_e('Informative (per category)', 'cdg-core'); ?></label><br>
                        <label><input type="radio" name="documentation_module_style" value="minimal" <?php checked($s['documentation_module_style'], 'minimal'); ?>> <?php esc_html_e('Minimal (single widget)', 'cdg-core'); ?></label><br><br>
                        <label><?php esc_html_e('Docs per widget:', 'cdg-core'); ?> <input type="number" name="documentation_widget_limit" value="<?php echo esc_attr($s['documentation_widget_limit']); ?>" min="1" max="20" style="width: 60px;"></label>
                    </div>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <label><input type="checkbox" name="enable_cpt_widgets" value="1" <?php checked($s['enable_cpt_widgets']); ?>> <?php esc_html_e('Enable CPT dashboard widgets', 'cdg-core'); ?></label>
                    <div style="margin: 10px 0 0 24px;">
                        <label><input type="checkbox" name="show_cpt_widgets" value="1" <?php checked($s['show_cpt_widgets']); ?>> <?php esc_html_e('Show on dashboard', 'cdg-core'); ?></label><br><br>
                        <strong><?php esc_html_e('Widget Style:', 'cdg-core'); ?></strong><br>
                        <label><input type="radio" name="cpt_module_style" value="informative" <?php checked($s['cpt_module_style'], 'informative'); ?>> <?php esc_html_e('Informative (per post type with stats)', 'cdg-core'); ?></label><br>
                        <label><input type="radio" name="cpt_module_style" value="minimal" <?php checked($s['cpt_module_style'], 'minimal'); ?>> <?php esc_html_e('Minimal (single quick-add widget)', 'cdg-core'); ?></label><br><br>
                        <strong><?php esc_html_e('Select Post Types:', 'cdg-core'); ?></strong><br>
                        <?php
                        $available_cpts = CDG_Core_CPT_Dashboard::get_available_post_types();
                        if (empty($available_cpts)): ?>
                            <p class="description"><?php esc_html_e('No custom post types available.', 'cdg-core'); ?></p>
                        <?php else:
                            foreach ($available_cpts as $pt): ?>
                                <label style="display:block;margin:3px 0;">
                                    <input type="checkbox" name="selected_cpts[]" value="<?php echo esc_attr($pt->name); ?>" <?php checked(in_array($pt->name, $s['selected_cpts'] ?? [], true)); ?>>
                                    <?php echo esc_html($pt->labels->name); ?>
                                </label>
                            <?php endforeach;
                        endif; ?>
                        <br>
                        <label><input type="checkbox" name="show_recent_posts" value="1" <?php checked($s['show_recent_posts']); ?>> <?php esc_html_e('Show recent posts in widgets', 'cdg-core'); ?></label><br>
                        <label><?php esc_html_e('Recent posts to show:', 'cdg-core'); ?> <input type="number" name="recent_posts_limit" value="<?php echo esc_attr($s['recent_posts_limit']); ?>" min="1" max="10" style="width: 60px;"></label>
                    </div>
                </td>
            </tr>
        </table>
    ...
    private function tab_cleanup(array $s): void
    {
            ...
            <tr>
                ...
                <td>
                    <?php foreach ([
                        ...
                        <label style="display:block;margin:3px 0;">
                            <input type="checkbox" name="<?php echo esc_attr($key); ?>" value="1" <?php checked($s[$key]); ?>>
        ...
        <table class="form-table">
            ...
            <tr>
                ...
                <td>
                    ...
                    $plugin_widgets = array_filter($plugin_widgets, function ($widget) use ($core_widgets) {
                        return !in_array($widget['id'], $core_widgets, true);
                    });

                    if (empty($plugin_widgets)): ?>
                        <p class="description">
                            <?php esc_html_e('No plugin widgets detected yet. Visit the Dashboard once to populate this list.', 'cdg-core'); ?>
                        </p>
                    <?php else: ?>
                        <?php foreach ($plugin_widgets as $widget): ?>
                            <label style="display:block;margin:3px 0;">
                                <input type="checkbox" name="hidden_dashboard_widgets[]" value="<?php echo esc_attr($widget['id']); ?>" <?php checked(in_array($widget['id'], $hidden_widgets, true)); ?>>
                                <?php echo esc_html($widget['title']); ?>
                                <code style="font-size: 11px; color: #666;"><?php echo esc_html($widget['id']); ?></code>
                            </label>
                        <?php endforeach; ?>
                        <p class="description" style="margin-top: 10px;">
                            <?php esc_html_e('These widgets were detected from other plugins. Check to hide them.', 'cdg-core'); ?>
                        </p>
                    <?php endif; ?>
                </td>
            </tr>
        ...
        <table class="form-table">
                ...
                <td>
                    <?php foreach ([
                        ...
                        <label style="display:block;">
                            <input type="radio" name="heartbeat_admin" value="<?php echo esc_attr($val); ?>" <?php checked($s['heartbeat_admin'], $val); ?>>
                            <?php echo esc_html($label); ?>
                        </label>
                    <?php endforeach; ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <?php foreach ([
                        ...
                        <label style="display:block;">
                            <input type="radio" name="heartbeat_frontend" value="<?php echo esc_attr($val); ?>" <?php checked($s['heartbeat_frontend'], $val); ?>>
                            <?php echo esc_html($label); ?>
                        </label>
                    <?php endforeach; ?>
                </td>
            ...
            <tr>
                ...
                <td>
                    <label style="display:block;">
                        <input type="checkbox" name="heartbeat_exception_builder" value="1" <?php checked($s['heartbeat_exception_builder']); ?>>
                        <?php esc_html_e('Divi Visual Builder', 'cdg-core'); ?>
                    </label>
                    <p class="description"><?php esc_html_e('When enabled, heartbeat will not be disabled while using the Divi Visual Builder.', 'cdg-core'); ?></p>
                </td>
            </tr>
        </table>
        <?php
    ...
    private function tab_security(array $s): void
    {
        ...
        <p class="description"><?php esc_html_e('Complements Wordfence and SpinupWP. Security headers (X-Frame-Options, HSTS, X-XSS-Protection, X-Content-Type-Options) are handled by SpinupWP at the Nginx level.', 'cdg-core'); ?></p>
        <table class="form-table">
            <?php foreach ([
                'disable_xmlrpc' => ['Disable XML-RPC', 'Common attack vector used for brute-force attacks'],
                'block_dangerous_uploads' => ['Block dangerous uploads', 'Prevents .exe, .php, .js and other executable files'],
                'remove_powered_by' => ['Remove X-Powered-By', 'Hides server information'],
                'disable_code_editor' => ['Disable code editor', 'For non-administrators only'],
            ] as $key => [$label, $desc]): ?>
                <tr>
                    <th><?php echo esc_html($label); ?></th>
                    <td>
                        <label><input type="checkbox" name="<?php echo esc_attr($key); ?>" value="1" <?php checked($s[$key]); ?>> <?php esc_html_e('Enable', 'cdg-core'); ?></label>
        ...
        <p class="description"><?php esc_html_e('Allow SVG file uploads. Use with caution as SVGs can contain malicious code.', 'cdg-core'); ?></p>
        <table class="form-table">
            <tr>
                <th><?php esc_html_e('Enable SVG Uploads', 'cdg-core'); ?></th>
                <td>
                    <label>
                        <input type="checkbox" name="enable_svg_uploads" value="1" <?php checked($s['enable_svg_uploads']); ?>>
                        <?php esc_html_e('Allow SVG file uploads', 'cdg-core'); ?>
                    </label>
                    <p class="description"><?php esc_html_e('When enabled, SVG files can be uploaded through the Media Library.', 'cdg-core'); ?></p>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <label>
                        <input type="checkbox" name="svg_admin_only" value="1" <?php checked($s['svg_admin_only']); ?>>
                        <?php esc_html_e('Only allow administrators to upload SVGs', 'cdg-core'); ?>
                    </label>
                    <p class="description"><?php esc_html_e('When enabled, only users with the manage_options capability (administrators) can upload SVG files.', 'cdg-core'); ?></p>
                </td>
            </tr>
        </table>
        <?php
    ...
    private function tab_performance(array $s): void
    {
            ...
            <tr>
                ...
                <td>
                    <?php foreach ([
                        ...
                        'optimize' => 'Remove CSS/JS on non-block pages (recommended)',
                        'disable' => 'Disable block editor',
                    ] as $val => $label): ?>
                        <label style="display:block;">
                            <input type="radio" name="gutenberg_mode" value="<?php echo esc_attr($val); ?>" <?php checked($s['gutenberg_mode'], $val); ?>>
                            <?php echo esc_html($label); ?>
                        </label>
                    <?php endforeach; ?>
                </td>
        ...
        <table class="form-table">
            <tr>
                ...
                <td>
                    <label style="display:block;"><input type="checkbox" name="optimize_search" value="1" <?php checked($s['optimize_search']); ?>> <?php esc_html_e('Search queries', 'cdg-core'); ?></label>
                    <label style="display:block;"><input type="checkbox" name="optimize_archives" value="1" <?php checked($s['optimize_archives']); ?>> <?php esc_html_e('Archive queries', 'cdg-core'); ?></label>
                </td>
            </tr>
        </table>
        ...
        <table class="form-table">
            <tr>
                ...
                <td><label><input type="checkbox" name="enable_lazy_loading" value="1" <?php checked($s['enable_lazy_loading']); ?>> <?php esc_html_e('Native lazy loading with aspect ratio (CLS)', 'cdg-core'); ?></label></td>
            </tr>
            <tr>
                ...
                <td>
                    ...
                    foreach ($sizes as $name => $data): ?>
                        <label style="display:block;margin:2px 0;">
                            <input type="checkbox" name="disabled_image_sizes[]" value="<?php echo esc_attr($name); ?>" <?php checked(in_array($name, $disabled, true)); ?>>
                            <?php echo esc_html("$name ({$data['width']}×{$data['height']})"); ?>
                        </label>
                    <?php endforeach; ?>
                    <br><label><input type="checkbox" name="remove_medium_large" value="1" <?php checked($s['remove_medium_large']); ?>> <?php esc_html_e('Always remove medium_large', 'cdg-core'); ?></label>
                </td>
            </tr>
        </table>
        ...
        <table class="form-table">
            <tr>
                ...
                <td>
                    <label style="display:block;"><input type="radio" name="post_revisions_mode" value="unlimited" <?php checked($s['post_revisions_mode'], 'unlimited'); ?>> <?php esc_html_e('Unlimited', 'cdg-core'); ?></label>
                    <label style="display:block;"><input type="radio" name="post_revisions_mode" value="disabled" <?php checked($s['post_revisions_mode'], 'disabled'); ?>> <?php esc_html_e('Disabled (no revisions saved)', 'cdg-core'); ?></label>
                    <label style="display:block;"><input type="radio" name="post_revisions_mode" value="limited" <?php checked($s['post_revisions_mode'], 'limited'); ?>> <?php esc_html_e('Limited:', 'cdg-core'); ?> <input type="number" name="post_revisions_limit" value="<?php echo esc_attr($s['post_revisions_limit']); ?>" min="1" max="100" style="width:60px;"> <?php esc_html_e('revisions per post', 'cdg-core'); ?></label>
                    <p class="description"><?php esc_html_e('Controls how many revisions WordPress keeps when saving posts and pages.', 'cdg-core'); ?></p>
                </td>
            </tr>
        </table>
        ...
        <table class="form-table">
            <tr>
                ...
                <td><label><input type="checkbox" name="remove_dns_prefetch" value="1" <?php checked($s['remove_dns_prefetch']); ?>> <?php esc_html_e('Remove s.w.org DNS prefetch', 'cdg-core'); ?></label></td>
            </tr>
        </table>
        <?php
    }
    ...
    {
        ?>
        <h2><?php esc_html_e('Gravity Forms / Divi Compatibility', 'cdg-core'); ?></h2>
        <?php if (!class_exists('GFForms')): ?>
            <div class="notice notice-info inline"><p><?php esc_html_e('Gravity Forms not active. Settings will apply when activated.', 'cdg-core'); ?></p></div>
        <?php endif; ?>
        <p class="description"><?php esc_html_e('Fixes "gf_global is not defined" errors caused by Divi script optimization.', 'cdg-core'); ?></p>
        <table class="form-table">
            <tr>
                <th><?php esc_html_e('Enable', 'cdg-core'); ?></th>
                <td><label><input type="checkbox" name="enable_gf_fixes" value="1" <?php checked($s['enable_gf_fixes']); ?>> <?php esc_html_e('Enable compatibility fixes', 'cdg-core'); ?></label></td>
            </tr>
            <tr>
                <th><?php esc_html_e('Detection', 'cdg-core'); ?></th>
                <td>
                    <label style="display:block;"><input type="radio" name="gf_detection_mode" value="auto" <?php checked($s['gf_detection_mode'], 'auto'); ?>> <?php esc_html_e('Auto-detect (recommended)', 'cdg-core'); ?></label>
                    <label style="display:block;"><input type="radio" name="gf_detection_mode" value="manual" <?php checked($s['gf_detection_mode'], 'manual'); ?>> <?php esc_html_e('Manual only', 'cdg-core'); ?></label>
                </td>
            </tr>
            <tr>
                <th><?php esc_html_e('Additional Pages', 'cdg-core'); ?></th>
                <td>
                    <textarea name="gf_manual_pages" rows="4" class="large-text code" placeholder="contact&#10;submit-form"><?php echo esc_textarea(implode("\n", $s['gf_manual_pages'] ?? [])); ?></textarea>
                    <p class="description"><?php esc_html_e('Page slugs, one per line. Always applies fixes to these.', 'cdg-core'); ?></p>
                </td>
            </tr>
        </table>
        <?php
    ...
    private function tab_admin(array $s): void
    {
        ...
        <table class="form-table">
            <tr>
                <th><?php esc_html_e('Enable', 'cdg-core'); ?></th>
                <td><label><input type="checkbox" name="enable_admin_branding" value="1" <?php checked($s['enable_admin_branding']); ?>> <?php esc_html_e('Show custom footer', 'cdg-core'); ?></label></td>
            </tr>
            <tr>
                <th><?php esc_html_e('Footer Text', 'cdg-core'); ?></th>
                <td>
                    <input type="text" name="admin_footer_text" value="<?php echo esc_attr($s['admin_footer_text']); ?>" class="large-text">
                    <p class="description"><?php esc_html_e('HTML allowed.', 'cdg-core'); ?></p>
                </td>
            </tr>
        </table>
        ...
        <table class="form-table">
            <tr>
                ...
                <td><textarea name="custom_admin_css" rows="10" class="large-text code"><?php echo esc_textarea($s['custom_admin_css']); ?></textarea></td>
            </tr>
        </table>
        <?php
    }
